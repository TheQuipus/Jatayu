...
type SeekerSocialAuthProps = {
  onSuccess: (response: AuthResponse) => void;
  onError: (message: string) => void;
  disabled?: boolean;
};

export default function SeekerSocialAuth({
  onSuccess,
  onError,
  disabled = false,
}: SeekerSocialAuthProps) {
  const { signInWithGoogle, isLoading: googleLoading, isAvailable: googleAvailable } = useGoogleAuth({
    role: "seeker",
    onSuccess,
    onError,
  });
  const { signInWithLinkedin, isLoading: linkedinLoading, isAvailable: linkedinAvailable } = useLinkedinAuth({
    role: "seeker",
    onSuccess,
    onError,
  });
  const busy = disabled || googleLoading || linkedinLoading;

  if (!googleAvailable && !linkedinAvailable) return null;
  ...
  return (
    <>
      ...
      <div className={styles.socialButtonsRow}>
        {googleAvailable ? <button type="button" className={styles.socialButton} onClick={signInWithGoogle} disabled={busy} aria-label="Continue with Google" title="Continue with Google">
        ...
        {linkedinAvailable ? <button type="button" className={styles.socialButton} onClick={signInWithLinkedin} disabled={busy} aria-label="Continue with LinkedIn" title="Continue with LinkedIn">
